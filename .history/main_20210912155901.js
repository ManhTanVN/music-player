...
let LOCAL_STORAGE_JOSE = {}
let musicIndex = Math.floor(Math.random() * allMusics.length + 1)

let config = JSON.parse(localStorage.getItem(LOCAL_STORAGE_JOSE)) || {}
function setConfig(key, value) {
    config[key] = value
    localStorage.setItem(LOCAL_STORAGE_JOSE, JSON.stringify(config))
}
...
repeatBtn.addEventListener('click', () => {
    ...
    switch(getText) {
       case 'repeat':
            ...
            setConfig('isRpeat2', true)
            setConfig('isShufle', false)
            repeatBtn.setAttribute('title', 'Song looped')
            break;
       case 'repeat_one':
            repeatBtn.innerText = 'shuffle'
            setConfig('isShuffle', true)
            setConfig('isRpeat2', false)
