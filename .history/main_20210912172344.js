...
function loadConfig() {
    ...
    let getDuration = ulTag.querySelector(`.no-${config.isPlaying - 1}`)
    // console.log(getDuration)
    getDuration.addEventListener('loadeddata', () => {
        let duration1 = getDuration.duration
        if(config.timeUpdate < duration1){
            mainAudio.currentTime = config.timeUpdate
            console.log('config: ', config.timeUpdate)
